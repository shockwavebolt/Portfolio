...
const Project = forwardRef(function Project(
  ...
  return (
    <div
      ref={ref}
      className={`col-span-full md:col-span-3 lg:col-span-4 w-full border-t-2 border-solid transition-colors duration-300 ${open ? "border-accent" : "border-secondary"} pt-4 grid grid-cols-subgrid`}
    >
      <div className="col-span-full md:col-start-1 md:col-span-2 flex flex-col gap-8">
        <div className="flex flex-col gap-4">
          <label
            className={`text-[14px] md:text-[16px] lg:text-[20px] font-ibm transition-colors duration-300 ${open ? "text-accent" : "text-secondary"}`}
          >
            {title}
          </label>
          {open && (
